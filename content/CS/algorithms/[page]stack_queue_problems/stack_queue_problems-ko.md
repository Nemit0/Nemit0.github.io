...
두 입력 문자열을 통해 스택 기반 알고리즘이 어떻게 동작하는지 단계별로 살펴봅니다.

**괄호 쌍**: `(` ↔ `)`, `{` ↔ `}`, `[` ↔ `]`

**알고리즘**: 여는 괄호는 모두 스택에 push합니다. 닫는 괄호가 나오면 스택의 top을 pop하고, 쌍이 맞으면 계속 진행하고 맞지 않으면 (또는 스택이 비어 있으면) **유효하지 않음**으로 판정합니다. 모든 문자를 처리한 후 스택이 **비어 있어야** 유효합니다.
...
| 2 | `{` | Push | `( {` | 여는 괄호 → push |
| 3 | `[` | Push | `( { [` | 여는 괄호 → push |
| 4 | `]` | `[` pop | `( {` | 닫는 괄호; top이 `[` → 쌍 일치 ✓ |
| 5 | `}` | `{` pop | `(` | 닫는 괄호; top이 `{` → 쌍 일치 ✓ |
| 6 | `)` | `(` pop | *(비어있음)* | 닫는 괄호; top이 `(` → 쌍 일치 ✓ |

모든 문자 처리 후 스택이 비어 있음 → **유효** ✅
...
| 2 | `{` | Push | `( {` | 여는 괄호 → push |
| 3 | `[` | Push | `( { [` | 여는 괄호 → push |
| 4 | `}` | `[` pop | — | 닫는 괄호; top이 `[`, 기대값은 `]` → **불일치** ✗ |

**4단계에서 실패**: 문자 `}`는 `{`를 닫아야 하지만, 스택의 top은 `[`입니다. 가장 안쪽의 여는 괄호 `[`가 닫히기 전에 `{`를 닫으려 했습니다. → **유효하지 않음** ❌

---

## 풀이 예제: 단조 스택 — 다음 큰 원소

**문제**: `arr = [2, 1, 5, 6, 2, 3]`이 주어질 때, 각 원소에 대해 오른쪽에서 자신보다 엄격하게 큰 첫 번째 원소를 구합니다. 없으면 `-1`입니다.

**알고리즘**: 아직 다음 큰 원소를 찾지 못한 **인덱스**를 스택에 저장합니다. 스택은 값 기준으로 **단조 감소** 상태를 유지합니다. 새 원소 `arr[i]`가 `arr[stack.top()]`보다 크면 pop하고 결과를 기록합니다.

**초기 상태**: `result = [-1, -1, -1, -1, -1, -1]`, stack = `[]`

| 단계 | 원소 `arr[i]` | 스택 이전 (인덱스) | 동작 | 스택 이후 (인덱스) | 결과 배열 변경 |
|------|--------------|-------------------|------|-------------------|---------------|
| i=0 | `2` | `[]` | 인덱스 0 push (아직 더 큰 원소 없음) | `[0]` | 변경 없음 |
| i=1 | `1` | `[0]` | `arr[0]=2 > 1` → pop 없음; 인덱스 1 push | `[0, 1]` | 변경 없음 |
| i=2 | `5` | `[0, 1]` | `arr[1]=1 < 5` → 1 pop: **result[1]=5**; `arr[0]=2 < 5` → 0 pop: **result[0]=5**; 인덱스 2 push | `[2]` | `result = [5, 5, -1, -1, -1, -1]` |
| i=3 | `6` | `[2]` | `arr[2]=5 < 6` → 2 pop: **result[2]=6**; 인덱스 3 push | `[3]` | `result = [5, 5, 6, -1, -1, -1]` |
| i=4 | `2` | `[3]` | `arr[3]=6 > 2` → pop 없음; 인덱스 4 push | `[3, 4]` | 변경 없음 |
| i=5 | `3` | `[3, 4]` | `arr[4]=2 < 3` → 4 pop: **result[4]=3**; `arr[3]=6 > 3` → pop 없음; 인덱스 5 push | `[3, 5]` | `result = [5, 5, 6, -1, 3, -1]` |
| 종료 | — | `[3, 5]` | 남은 인덱스 3, 5는 다음 큰 원소 없음 → `-1` 유지 | `[]` | `result = [5, 5, 6, -1, 3, -1]` |

**최종 결과**: `[5, 5, 6, -1, 3, -1]`

의미:
- `arr[0]=2` → 다음 큰 원소는 `5` (인덱스 2)
- `arr[1]=1` → 다음 큰 원소는 `5` (인덱스 2)
- `arr[2]=5` → 다음 큰 원소는 `6` (인덱스 3)
- `arr[3]=6` → 다음 큰 원소 없음 → `-1`
- `arr[4]=2` → 다음 큰 원소는 `3` (인덱스 5)
- `arr[5]=3` → 다음 큰 원소 없음 → `-1`

### 왜 O(n)인가

각 원소(인덱스)는 스택에 **정확히 한 번 push**되고 **최대 한 번 pop**됩니다. 따라서 알고리즘 전체에서 push + pop 연산의 총 횟수는 `2n`을 넘지 않습니다. 중첩된 `while` 루프가 있더라도 전체 실행에서 그 반복 횟수의 합은 `n`을 초과할 수 없습니다. 이로 인해 전체 시간 복잡도는 O(n²)이 아닌 **O(n)**입니다.
